# Symphony vs Sprints Component Audit

This audit compares the pasted OpenAI/Symphony system overview against the
current Sprints implementation in this repository.

Summary: Sprints is architecturally aligned with Symphony, but it is not a
...
- `change-delivery` extends beyond Symphony:
  ...
  attention.
- Sprints persists orchestration state in SQLite plus JSON/JSONL projections.
  Symphony's overview describes mainly in-memory runtime state plus logs.
- Sprints is tracker-neutral in code, but GitHub is the supported production
...
- `sprints/workflows/contract.py`

Difference:

Sprints is more flexible than the reference loader, but the projected
...
- `sprints/workflows/validation.py`
- `sprints/workflows/issue_runner/preflight.py`
- `sprints/workflows/issue_runner/workspace.py`
- `sprints/trackers/__init__.py`

Difference:

Sprints validates strongly, but its config access is still distributed across
...
- `sprints/trackers/__init__.py`
- `sprints/trackers/github.py`
- `sprints/trackers/linear.py`
- `sprints/trackers/local_json.py`
- `sprints/workflows/issue_runner/tracker.py`

Difference:

Symphony is Linear-first. Sprints is tracker-neutral in shape but GitHub-first
...
- `sprints/workflows/issue_runner/orchestrator.py`
- `sprints/workflows/issue_runner/workspace.py`
- `sprints/engine/store.py`
- `sprints/engine/scheduler.py`
- `sprints/engine/lifecycle.py`

Difference:

Sprints is more durable than the reference overview. It treats SQLite as the
...
- It writes prompt/output under `.sprints/`.
- It supports hooks:
  - `after_create`
  - `before_run`
  - `after_run`
...
- `sprints/runtimes/__init__.py`
- `sprints/runtimes/stages.py`
- `sprints/runtimes/codex_app_server.py`
- `sprints/runtimes/capabilities.py`
- `sprints/workflows/issue_runner/workspace.py`

Difference:

Symphony's overview centers on one coding-agent app-server executable. Sprints
...
- `sprints/workflows/change_delivery/server/`
- `sprints/formatters.py`
- `sprints/watch.py`
- `sprints/watch_sources.py`

Difference:

Sprints is stronger than the reference here. It has a larger operator control
...
- Writes Sprints runtime event JSONL.
...
- `sprints/engine/store.py`
- `sprints/engine/audit.py`
- `sprints/workflows/change_delivery/webhooks/`
- `sprints/workflows/issue_runner/workspace.py`

Difference:

Sprints logging is not only sink-based; it is also part of the durable engine
event model.
...
Sprints supports the same idea, but `change-delivery` has much more policy in
...
This is one of the clearest conformance gaps. Sprints should centralize this
...
Sprints implements the layer, but it is split between generic engine primitives
...
Sprints is broader and more plugin-like.
...
Sprints splits tracker and code-host concerns. That is more suitable for
...
Sprints is significantly more developed here.
...
- Public contract is still Sprints-shaped, not purely Symphony-shaped.
...
This should be documented as a Sprints extension rather than forced into the
...
2. Move Sprints-specific config under `sprints:` where possible.
3. Add or formalize a typed config layer with consistent defaults, environment
   indirection, and path normalization.
4. Decide whether strict compatibility means Linear-first, or whether Sprints
...
Sprints is a durable, production-oriented workflow engine inspired by the
...
- Sprints implements a broader durable orchestration system with multiple
...
> Sprints is Symphony-inspired and partially compatible. `issue-runner` is the
