...
A **runtime** is the thing Sprints shells out to when a turn happens. Sprints owns leases, state, and dispatch; the runtime owns "how do I actually run an LLM turn against a worktree." Four are shipped today.

At the code level, these shared execution backends live under
`sprints/runtimes/`. The operator-facing contract also uses the `runtimes:`
...
through `sprints/runtimes/stages.py`. The shared stage dispatcher owns the
execution boundary:
...
Sprints validates runtime wiring through a shared stage contract before the
service starts:
...
|---|---|---|---|
| `hermes-agent` | `hermes -z` or `hermes chat --quiet -q` | one-shot from Sprints' point of view | `prompt-turn`, `command-stage`, `one-shot`, `activity-heartbeat` |
...
Sprints validates runtime bindings before dispatch. The checks cover:
...
If the selected runtime lacks one of those capabilities, `hermes sprints
...
For common choices, let Sprints edit the repo-owned workflow contract:
...
hermes sprints runtime-matrix --format json
```
...
hermes sprints runtime-matrix --role agent --execute
hermes sprints runtime-matrix --runtime codex-app-server --execute --format json
```
...
listener first with `hermes sprints codex-app-server up`.
...
placeholders plus `SPRINTS_*` environment variables. If the command writes a
JSON object to `{result_path}`, Sprints records its `output`, `session_id`,
...
For a supervised long-running listener, install and start the Sprints-managed
user service:
...
hermes sprints codex-app-server logs
```
...
Sprints presents `Authorization: Bearer <token>` during the WebSocket
...
Then configure Sprints for external mode:
...
transport cannot be shared: Sprints can only attach to an already-started
...
It maps `thread/tokenUsage/updated` into Sprints token totals and
`account/rateLimits/updated` into the latest rate-limit snapshot. When Codex
emits both `tokenUsage.last` and cumulative `tokenUsage.total`, Sprints records
...
- Protocol: `sprints/runtimes/__init__.py`
- Stage dispatcher: `sprints/runtimes/stages.py`
- Adapters: `sprints/runtimes/{claude_cli,acpx_codex,hermes_agent,codex_app_server}.py`
- Runtime adapters: `sprints/runtimes/`
- Preflight: `sprints/workflows/change_delivery/preflight.py`
