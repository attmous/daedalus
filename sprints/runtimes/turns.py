"""Run one Sprints actor turn through a configured runtime.

A Sprints turn is one prompt/result exchange at the runtime boundary. Some
    ...
    *,
    ...
    try:
        ...
        if command is not None:
            ...
            stage_env.setdefault("SPRINTS_PROMPT_PATH", str(resolved_prompt_path))
            stage_env.setdefault("SPRINTS_RESULT_PATH", str(resolved_result_path))
            stage_env.setdefault("SPRINTS_WORKTREE", str(worktree))
            stage_env.setdefault("SPRINTS_SESSION_NAME", session_name)
            stage_env.setdefault("SPRINTS_MODEL", model)
