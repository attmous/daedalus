...
DEFAULT_INSTANCE_ID = "sprints-plugin"
...
def _absolute_secret_path(value: str, *, flag_name: str) -> str:
    ...
    if not path.is_absolute():
        raise SprintsCommandError(f"{flag_name} must be an absolute path")
    return str(path)
...
def _codex_app_server_ws_auth_args(
    *,
    ...
    if token_file and token_sha256:
        raise SprintsCommandError("use either --ws-token-file or --ws-token-sha256, not both")
    if (token_file or token_sha256) and shared_secret_file:
        raise SprintsCommandError("capability-token and signed-bearer-token auth modes are mutually exclusive")
    if (issuer or audience or ws_max_clock_skew_seconds is not None) and not shared_secret_file:
        raise SprintsCommandError("--ws-issuer, --ws-audience, and --ws-max-clock-skew-seconds require --ws-shared-secret-file")
    if ws_max_clock_skew_seconds is not None and ws_max_clock_skew_seconds < 0:
        raise SprintsCommandError("--ws-max-clock-skew-seconds must be non-negative")
...
def _workflow_name_for_root(workflow_root: Path) -> str:
    ...
    if not workflow_name:
        raise SprintsCommandError(f"{contract.source_path} is missing top-level `workflow:` field")
...
def _load_workflow_module_for_root(workflow_root: Path):
    ...
    try:
        ...
        raise SprintsCommandError("unable to load workflow dispatcher") from exc
    try:
        return load_workflow(workflow_name)
    except Exception as exc:
        raise SprintsCommandError(f"unable to load workflow {workflow_name!r}: {exc}") from exc


def _load_issue_runner_workspace(workflow_root: Path):
    raise SprintsCommandError("issue-runner was removed; use workflow: agentic")


def _load_change_delivery_workspace(workflow_root: Path):
    raise SprintsCommandError("change-delivery was removed; use workflow: agentic")
...
def build_runs_report(
    *,
    ...
    if action == "show":
        if not run_id:
            raise SprintsCommandError("runs show requires a run_id")
        run = read_engine_run(db_path, workflow=workflow_name, run_id=run_id)
        if run is None:
            raise SprintsCommandError(f"unknown engine run: {run_id}")
...
def codex_app_server_doctor(
    *,
    ...
    if mode not in {"managed", "external"}:
        raise SprintsCommandError("--mode must be managed or external")
...
def _workflow_template_path(workflow_name: str) -> Path:
    ...
    if path is None:
        raise SprintsCommandError(f"no bundled workflow template for {workflow_name!r}")
    return path
...
def _git_stdout(*args: str, cwd: Path) -> str:
    ...
    if completed.returncode != 0:
        ...
        raise SprintsCommandError(f"`git {' '.join(args)}` failed in {cwd}: {detail}")
...
def _discover_git_repo_root(start_path: Path | None) -> Path:
    ...
    if not start.exists():
        raise SprintsCommandError(f"repo path does not exist: {start}")
    ...
    *,
    ...
    for path in paths:
        ...
        try:
            ...
            raise SprintsCommandError(f"cannot commit path outside repo root: {resolved}") from exc
...
def bootstrap_workflow_root(
    *,
    ...
    try:
        ...
        raise SprintsCommandError(f"--repo-slug {resolved_repo_slug!r} is invalid: {exc}") from exc
...
def scaffold_workflow_root(
    *,
    ...
    try:
        ...
        raise SprintsCommandError(f"unable to load workflow template {template_path}: {exc}") from exc
    config = dict(template_contract.config)
    workflow_policy = template_contract.prompt_template

    resolved_repo_slug = repo_slug.strip()
    if not resolved_repo_slug:
        raise SprintsCommandError("--repo-slug cannot be blank")
    try:
